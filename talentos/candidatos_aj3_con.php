<?php
//
//- candidatos_aj3_con.php | CURRÍCULO | Salva ALTERAÇÃO de CURSO (formação acadêmica) 
//- (C)haia, 26/03/2025 | (U) 28/04/2026
//
...
if (empty($idConq) || empty($idConqTipo) || empty($ano) || empty($titulo) || empty($descricao)) {
    $retorno = [
        "status" => false,
...
// alterar conquistas/certificados de qualquer outro candidato só sabendo o id).
$sql = "SELECT C.*, P.nome, D.arquivo
            FROM rh_cv_conq C
            INNER JOIN rh_pessoas P on P.idPessoa = C.idPessoa
            LEFT OUTER JOIN rh_documentos D ON D.idDoc = C.idDoc
            WHERE C.id = :idConq AND C.idPessoa = :idPessoa";
$stmt = $conn->prepare($sql);
$stmt->bindParam(':idConq', $idConq, PDO::PARAM_INT);
...
if (isset($_FILES['arquivoCert']) && $_FILES['arquivoCert']['error'] === UPLOAD_ERR_OK) {
    ...
    $sql = "UPDATE rh_cv_conq SET idConqTipo = :idConqTipo, titulo = :titulo, ano = :ano,
                descricao = :descricao, idDoc = $idDoc WHERE id = :idConq AND idPessoa = :idPessoa";
} else {
    $sql = "UPDATE rh_cv_conq SET idConqTipo = :idConqTipo, titulo = :titulo, ano = :ano,
                descricao = :descricao WHERE id = :idConq AND idPessoa = :idPessoa";
}
$consulta = $conn->prepare($sql);
$consulta->bindParam(':idConq', $idConq, PDO::PARAM_INT);
