<?php
//
//- rh_rescisao_aj2.php | Salva Registro da RESCISÃO (EDIÇÃO)
//- (C)haia, 24/04/2025
//
...
/*
 rh_rescisao_aj4.php | 2025-04-25 10:21:58 
{
    "nmPessoa": "Jos\u00e9 Paulo Toledo",
    "idColab": "10",
    "idPessoa": "34",
    "idRescisao": "2",
    "idTipoRescisao": "1",
    ...
    "_status": "Pendente",
    "_saldoSalario": "5000.00",
    "_feriasVencidas": "5000.00",
    "_feriasProporcionais": "1540.00",
    "_decimoTerceiro": "2500.00",
    "_multaFgts": "4300.00",
    "_descontos": "4000.00",
    "_totalLiquido": "14340.00",
    "_motivo": "<p>Colaborador vem apresentando des\u00eddia no local de trabalho, sendo rude com os colegas, etc.<\/p>"
}
*/
...
$sql = "SELECT * FROM rh_rescisoes WHERE idRescisao = :idRescisao";
$stmt = $conn->prepare($sql);
$stmt->bindValue(':idRescisao', $_POST['idRescisao'], PDO::PARAM_INT);
$stmt->execute();
$linha = $stmt->fetch(PDO::FETCH_ASSOC);
$antigos = json_encode($linha);

//
//- VERIFICA SE MUDOU DE COLABORADOR
//
if ($linha['idColab'] != $idColab) {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bindValue(':idColab', $linha['idColab'], PDO::PARAM_INT);
    $stmt->execute();
}

//-- ATUALIZA O REGISTRO
//
$sql = "UPDATE rh_rescisoes SET 
            idColab = :idColab,
            idTipoRescisao = :idTipoRescisao,
...
// Executa a query
if ($stmt->execute()) {
    $msg = "<div class='alert alert-success'><strong>Successo!</strong> ao atualizar registro. ID: $idRescisao</div>";
    ...
    if ($idTipoRescisao == 2) $idStatus = 8;  // Pediu demissão
...
f_log("ALT", "ALTERAÇÃO de RESCISÃO de $nmPessoa: Dados Antigos: ($antigos) | Dados Novo:( $dados )", "rh_rescisoes", $idModulo, $idRescisao);
