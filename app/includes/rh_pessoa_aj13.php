<?php
//
//- rh_pessoa_a13.php | INCLUI novo ARQUIVO - FOTOS
//- (C)haia, 07/03/2025
//
...
if (!empty($_FILES['foto_arquivo']['name'])) {
    ...
    if (move_uploaded_file($fileTmpPath, $destPath)) {
        ...
        $validade = new DateTime($foto_data); // Converter a string em um objeto DateTime
        ...
        $stmt->bindParam(':data', $foto_data, PDO::PARAM_STR);
        ...
        f_log("INC", "INCLUSÃO de Documento: Foto - Dados( $fileName )", "rh_documentos", $idModulo, $idDoc);
        //
        $tipo = 14; // Documento anexado
        $descricao = "Inserido foto da pessoa ($fileName)";
        f_ldt( $tipo, $idPessoa, $descricao);
        //
        $retorno = [
            "status" => true,
