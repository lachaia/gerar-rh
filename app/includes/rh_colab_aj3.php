<?php
// rh_colab_aj3.php - Upload de arquivo da CTPS
// (C) 2025
...
// Validações
if (!isset($_FILES['arquivo_ctps']) || $_FILES['arquivo_ctps']['error'] != 0) {
...
$nomeArquivo = "ctps_" . $idPessoa . "_" . time() . "." . $extensao;
...
if (move_uploaded_file($arquivo['tmp_name'], $caminhoCompleto)) {
    ...
    // Atualiza no banco
    //
    $sql = "UPDATE rh_colaboradores SET arquivo_ctps = ? WHERE idPessoa = ?";
    $stmt = $conn->prepare($sql);
    ...
    if( $substituido == 0 ) $descricao = "Inserido arquivo Cópia da CTPS: $nomeArquivo";
        else $descricao = "Substituído arquivo Cópia da CTPS: $nomeArquivo";
    ...
    $idTipoDoc = 5; // Tipo: 5 --> Cópia da CTPS
    ...
    f_log("INC", "INCLUSÃO de Documento: Cópia da CTPS de $nmPessoa - Dados( $original )", "rh_documentos", $idModulo, $idDoc);
    //
