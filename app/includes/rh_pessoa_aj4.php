<?php
//
//- rh_pessoa_aj4.php | Insere Nova PESSOA na tabela - COMPROVANTE DE ENDEREÇOS
//- (C)haia, 03/03/2025
//
...
if ($idPessoa == 0) {
    ...
    if ($stmt->execute()) {
        ...
        f_log("INC", "INCLUSÃO de Pessoa: Dados( $dados_2 )", "rh_pessoas", $idModulo, $idPessoa);
        //
...
if (!empty($_FILES['ce_arquivo']['name'])) {
    $validacao = upload_seguro_validar($_FILES['ce_arquivo'], ['pdf', 'jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp']);
    if ($validacao !== true) {
        $conn = null;
