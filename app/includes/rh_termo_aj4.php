<?php
//
//- rh_termo_aj4.php | EXCLUIR Registro do TERMO
//- (C)haia, 11/08/2025
//
...
$sql = "DELETE FROM rh_termos WHERE id = :id";
$stmt = $conn->prepare($sql);
$stmt->execute(['id' => $id]); 
...
if ($stmt->execute()) {
    ...
    //
        $sql = "DELETE FROM rh_pessoas_ldt WHERE origem='TRM' and idOrigem=$id";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
...
f_log("EXC", "EXCLUSÃO de TERMO no Sistema, ID: $id | Dados( $stringDados )", "rh_termos", $idModulo, $id);
