<?php
//
//- rh_cv_con_aj3.php | CURRÍCULO | Salva nova CONQUISTA/CERTIFICADO) 
//- (C)haia, 31/03/2025
//
...
*/
if (empty($idPessoa) || empty($idConqTipo) || empty($ano) || empty($titulo) || empty($descricao)) {
    $retorno = [
        "status" => false,
...
if ($consulta->execute()) {
    ...
    //
    $sql = "SELECT * FROM rh_cv_conq WHERE id = $idCon";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
