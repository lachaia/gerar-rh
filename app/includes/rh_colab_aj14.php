<?php
//
//- rh_colab_aj14.php | Exclui arquivo de CTPS
//- (C)haia, 15/04/2025
//
...
$sql = "SELECT arquivo_ctps, idPessoa
...
$arquivo_ctps = $dados['arquivo_ctps'];
$idPessoa = $dados['idPessoa'];

if( ! empty($arquivo_ctps)){
    $caminho = "docs/pessoa_$idPessoa/$arquivo_ctps";
    if (file_exists($caminho)) {
        unlink($caminho); // Exclui o arquivo
    }
    //
    $sql = "UPDATE rh_colaboradores SET arquivo_ctps = NULL WHERE idColab = $idColab";
    $stmt = $conn->prepare($sql);  
    $stmt->execute();
    ...
    $descricao = "Excluiu o arquivo CTPS";
