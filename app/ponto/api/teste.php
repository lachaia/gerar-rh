...
$sql = "SELECT distinct H.colaborador_id, P.nome, C.horario_ini, C.horario_fim, S.cidade AS cidade_id, S.estado AS colaborador_uf
            ...
            WHERE C.data_rescisao IS NULL AND H.colaborador_id = 3
            ORDER BY P.nome";
...
foreach ($colaboradores as $colaborador) {
    ...
    $sql = "SELECT 
                    ...
                    WHERE colaborador_id = $colaborador_id AND year( data_ref ) = 2025 AND month( data_ref ) = 11
                    ORDER BY 1,2,3";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    ...
    foreach ($meses as $m) {
        ...
        if (in_array($m, $meses_rejeitados)) {
        ...
        <div class="container">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    $sql = "SELECT * 
                            FROM rh_ponto_banco_horas H
                            WHERE colaborador_id = 1 and year( data_ref ) = $ano and month( data_ref ) = $mes
                            ORDER BY data_ref";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute();
