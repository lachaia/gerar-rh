        ...
        $(selector).summernote({
            ...
            callbacks: {
                onInit: function () {
                    ...
                    // borda vermelha espessa na caixa de edição
                    $(selector).next('.note-editor')
...
function constroiTabelaMembros() {
    ...
    dataTableMembros = new DataTable('#tabMembros', {
        ...
        "serverSide": false,
        "order": [
            [5, "desc"], // 1ª ordenação: coluna 5 (Vcto) de forma decrescente
            [1, "asc"]   // 2ª ordenação: coluna 1 (Nome) de forma crescente
        ],
        "scrollX": true,
...
function f_excluir_membro(idMembro) {
    if (confirm("Confirma a Exclusão do Membro " + idMembro + " ?") == true) {
        $.post("rh_cipa_aj4.php", { id: idMembro },
...
function f_ver_membro(id) {
    $.post("rh_cipa_aj5.php", { id: id }, function (retorno) {
        // alert(retorno);
        const x = JSON.parse(retorno);
        //
        $("#v_id").html(x.dados.idMembro);
        $("#v_nome").html(x.dados.nome);
        $("#v_cargo").html(x.dados.dsCargo);
        $("#v_subsede").html(x.dados.dsSubSede);
        $("#v_email").html(x.dados.email);
        $("#v_telefone").html(x.dados.telefone);
...
function f_editar_membro($idMembro) {
    $("#modalIncluirMembro").html("");
    //
    $.post("rh_cipa_aj5.php", { id: $idMembro }, function (retorno) {
        const x = JSON.parse(retorno);
        //
...
function btnIncAtendimento() {
    ...
    // Carregar o seletor de cipeiros via AJAX
    $.ajax({
        url: 'rh_cipa_aj24.php',
        type: 'GET',
        success: function (data) {
            $('#divIncAtendCipeiros').html(data);
        },
        error: function (xhr, status, error) {
            $('#divIncAtendCipeiros').html('<div class="alert alert-danger">Erro ao carregar cipeiros.</div>');
            console.error('Erro ao carregar cipeiros:', error);
        }
    });
}
...
function btnSalvarTipo() {
    ...
    $.post("rh_cipa_aj7.php", { descricao: descricao }, function (res) {
        alert(res.msg);
        if (res.status) {
...
function btnSalvarAtendimento() {
    ...
    if (idMembro.val() === "" || idMembro.val() === "0") {
        alert("Por favor, informe o nome do cipeiro.");
        $("#idMembro").focus();
        return false;
    }
...
function f_ver_atende(id) {
    $.post("rh_cipa_aj10.php", { id: id }, function (retorno) {
        const x = JSON.parse(retorno);
        //
        $("#v_data_ocorrencia").html(x.dados.data_ocorrencia);
        $("#v_nome_cipeiro").html(x.dados.nmCipeiro);
...
function f_editar_atende(id) {
    ...
    $.post("rh_cipa_aj10.php", { id: id }, function (retorno) {
        ...
        $("#formAltAtendimento #idMembro").val(x.dados.idCipeiro);
...
let cipeirosSelecionados = [];
let altCipeirosSelecionados = [];
let acaoCipeirosSelecionados = [];

function addCipeiro(formName = "#formIncReuniao") {
    let select = $(`${formName} #cipeiro`);
    let id = select.val();
    let nome = select.find("option:selected:not([disabled])").text().trim();

    if (!id || id === "0" || id === "") {
        alert("Por favor, selecione um Cipeiro válido.");
        return;
    }

    let arrSelecionados = formName === "#formAltReuniao" ? altCipeirosSelecionados : cipeirosSelecionados;
    ...
    span.innerHTML = `
        ${nome}
        <i class='fa-solid fa-xmark ms-2' style='cursor:pointer' onclick='removeCipeiro("${id}", "${formName}")'></i>
        <input type='hidden' name='idCipeiro[]' value='${id}'>
    `;

    $(`${formName} #cipeirosContainer`).append(span);

    // Restaura o select para a primeira opção ("Selecione...")
    select.prop("selectedIndex", 0);
}

function addTodosCipeiro(formName = "#formIncReuniao") {
    //
    $.ajax({
        url: "rh_cipa_aj14.php",
        method: "POST",
        dataType: "json",
        success: function (response) {
            if (!response.status || !Array.isArray(response.dados)) {
                alert("Erro ao carregar Cipeiros!");
                return;
            }

            response.dados.forEach(b => {
                let id = b.idCipeiro.toString();
                let nome = b.nome;

                // Seleciona o array correto
                let listaSelecionados = formName === "#formAltReuniao" ? altCipeirosSelecionados : cipeirosSelecionados;
...
function removeCipeiro(id, formName = "#formIncReuniao") {
    //
    if (formName === "#formAltReuniao") {
        altCipeirosSelecionados = altCipeirosSelecionados.filter(b => b !== id);
    } else {
        cipeirosSelecionados = cipeirosSelecionados.filter(b => b !== id);
    }

    $(`${formName} #cipeirosContainer input[value='${id}']`).closest("span").remove();
}
...
function btnSalvarReuniao(e) {
    ...
    $("input[name='idCipeiro[]']").each(function () {
        cipeiros.push($(this).val());
    });
    ...
    if (cipeiros.length === 0) {
        alert("Por favor, selecione pelo menos um Cipeiro presente.");
        return;
    }
    ...
    //
    mensagem.html('Aguarde, salvando os dados...');
    $("#divBtnSalvarReuniao").hide();
    //
    $.ajax({
        url: "rh_cipa_aj15.php",
        type: "POST",
        data: formData,
        dataType: "json",
        ...
        success: function (response) {
            ...
            if (response.status) {
                ...
                setTimeout(function () {
                    ...
                    $("#cipeirosContainer").empty();
                    incModalReuniao.hide();
                    dataTableReunioes.clear().draw();
                    dataTableReunioes.destroy();
                    constroiTabelaReunioes();
                    $("#divBtnSalvarReuniao").show();
                }, 3000);
            } else {
...
function f_ver_reuniao(id) {
    $.post("rh_cipa_aj17.php", { id: id }, function (retorno) {
        const x = JSON.parse(retorno);
        //
        ...
        $("#formVerReuniao #v_ata_arquivo").html(x.dados.ata_arquivo ? `<a href="#" onclick="f_ver_ata('${x.dados.ata_arquivo}')"><i class="fa-solid fa-file-pdf"></i> Ver Ata</a> ${x.dados.arquivoOriginal}` : 'Nenhum arquivo anexado');
        $("#formVerReuniao #v_cipeiros").html(x.dados.nomesParticipantes);
...
function f_editar_reuniao(id) {
    // Zera os Cipeiros selecionados
    altCipeirosSelecionados = [];
    ...
    $.post("rh_cipa_aj17.php", { id: id }, function (retorno) {
        ...
        // Limpa o container visual dos Cipeiros
        $("#formAltReuniao #cipeirosContainer").empty();

        // Monta visualmente os Cipeiros
        if (x.dados.idParticipantes && x.dados.nomesParticipantes) {
            ...
            ids.forEach((id, index) => {
                ...
                altCipeirosSelecionados.push(id);
                ...
                `;
                $("#formAltReuniao #cipeirosContainer").append(span);
            });
        }
    });
...
function btnSalvarAltReuniao(e) {
    ...
    $("#formAltReuniao input[name='idCipeiro[]']").each(function () {
        cipeiros.push($(this).val());
    });
    ...
    mensagem.html('Aguarde, salvando os dados...');
    $("#divBtnSalvarAltReuniao").hide();

    $.ajax({
        url: "rh_cipa_aj18.php",
        type: "POST",
        data: formData,
        dataType: "json",
        processData: false,
...
function btnIncAcao() {
    let container = $("#formIncAcao #cipeirosContainer");
    $("#formIncAcao")[0].reset();
    container.html('');
    incModalAcao.show();
}
...
function btnSalvarAcao() {
    ...
    //
    mensagem.html('Aguarde, salvando os dados...');
    $("#divBtnSalvarAcao").hide();
    //
    $.ajax({
        url: "rh_cipa_aj20.php",
        type: "POST",
        data: formData,
        dataType: "json",
        processData: false, // necessário para envio de arquivos
...
function f_ver_acao(id) {
    $.post("rh_cipa_aj21.php", { id: id }, function (retorno) {
        const x = JSON.parse(retorno);
        //
        ...
        $("#formVerAcao #v_acao_arquivo").html(x.dados.acao_arquivo ? `<a href="#" onclick="f_ver_ata('${x.dados.acao_arquivo}')"><i class="fa-solid fa-file-pdf"></i> Ver Documento</a>` : 'Nenhum arquivo anexado');
        $("#formVerAcao #v_cipeiros").html(x.dados.nomesParticipantes);
...
function f_editar_acao(id) {
    ...
    $.post("rh_cipa_aj21.php", { id: id }, function (retorno) {
        ...
        $("#formAltAcao #idSubSede").val(x.dados.idSubSede);
        ...
        $("#formAltAcao #cipeirosContainer").empty();
        ...
        if (x.dados.idParticipantes && x.dados.nomesParticipantes) {
            ...
            ids.forEach((id, index) => {
                ...
                `;
                $("#formAltAcao #cipeirosContainer").append(span);
            });
        }
    });
    altModalAcao.show();
...
function btnSalvarAltAcao() {
    ...
    $("#formAltAcao input[name='idCipeiro[]']").each(function () {
        cipeiros.push($(this).val());
    });
    ...
    mensagem.html('Aguarde, salvando os dados...');
    $("#divBtnSalvarAltAcao").hide();

    $.ajax({
        url: "rh_cipa_aj22.php",
        type: "POST",
        data: formData,
        dataType: "json",
        processData: false,
...
function f_ver_doc(idDoc) {
    //
    $.post("rh_cipa_aj26.php", { id: idDoc }, function (retorno) {
        const x = JSON.parse(retorno);
...
function btnIncDoc() {
    $("#formIncDoc")[0].reset();
    $.post("rh_cipa_aj28.php", { id: 0 }, function (retorno) {
...
function f_excluir_doc(id) {
    ...
    if (confirm("Confirma a Exclusão do Documento de CIPA ID: " + id + " ?") == true) {
        $.post("rh_cipa_aj29.php", { id: id },
            function (retorno, status) {
                const dados = JSON.parse(retorno);
                mensagem.html(dados.msg);
                //
...
function f_editar_doc(id) {
    //
    //
    $.post("rh_cipa_aj26.php", { id: id }, function (retorno) {
        const x = JSON.parse(retorno);
        ...
        //
        $.post("rh_cipa_aj28.php", { id: x.dados.idTipoDoc }, function (retorno) {
...
function btnIncSalvarUsuario() {
    ...
    //
    var chkCipa    = 1;     //- SIM -->  é membro da CIPA por padrão
    var chkBrigada = 0;     //- Não é membro da Brigada
    //
    if (!idPessoa || isNaN(idPessoa) || Number(idPessoa) <= 0) {
        alert("Por favor, Selecione uma Pessoa...");
        $("#inputIDPessoa").focus();
