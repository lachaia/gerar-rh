<?PHP
//
//- rh_cipa_aj20.php | Grava Inclusão de AÇÃO de CIPA
// (C)haia, 30/06/2025
...
if( $dados ){
    ...
    $idEmpresa = $_SESSION['idEmpresa'] ?? 0; // Pega a Empresa do usuário logado
    $idPessoa = $_SESSION['idPessoa'] ?? 0; // Pega a Pessoa do usuário logado
    //    
}else{
...
die;
/*
 rh_cipa_aj20.php | 2025-08-14 11:07:42 
{
    "data_acao": "2025-08-14",
    "idSubSede": "104",
    "assunto": "Viagem a negocios",
    "observacoes": "<p>A CIPA foi para Guarapuava para fazer uma ação de mapeamento de pontos de risco de segurança do trabalhador ...<\/p>",
    "idCipeiro": [
        "4",
        "7",
        "14",
...
try {
    $sql = "INSERT INTO rh_cipa_acoes (idSubSede, data_acao, assunto, observacoes, criado_por, idLogin) 
    ...
    if ($result) {
        ...
        f_log("INC", "Incluiu Ação de CIPA: $stringDados", "rh_cipa_acoes", $idModulo, $idAcao);
            ...
            foreach ($ids as $idCipeiro) {
                ...
                if (is_numeric($idCipeiro) && $idCipeiro > 0) {
                    ...
                    if (!$stmt->execute()) {
                        ...
                        f_log("ERR", "Erro ao inserir membro na ação: " . implode(", ", $stmt->errorInfo()), "rh_cipa_acao_membros", $idModulo, $idReuniao);
                    }
                }
            }   
        ...
        //
            foreach ($ids as $idCipeiro) {
                ...
                if (is_numeric($idCipeiro) && $idCipeiro > 0) {
                    ...
                    $descricao = "Participou da Ação de CIPA: $assunto em $data_acao";
        ...
        //
            ...
            if (isset($_FILES['acao_arquivo']) && $_FILES['acao_arquivo']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($arquivoTmp, $caminhoFinal)) {
                    ...
                    //
                        $idTipoDoc = 25; //- Documentos de CIPA
                        $assunto = "Ação de CIPA: $assunto";
                    //
