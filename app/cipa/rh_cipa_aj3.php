<?PHP
//
//- rh_cipa_aj3.php | Grava Inclusão de Membro de CIPA
// (C)haia, 17/06/2025
...
$conn = null;
die;
 rh_cipa_aj3.php | 2025-06-17 11:41:58 
{
...
try {
    $sql = "INSERT INTO rh_cipeiros (idPessoa, data_inicio, data_final, idCargo, idSubSede, criado_por, idLogin) 
                        VALUES ( :idPessoa, :data_inicio, :data_final, :idCargo, :idSubSede, :criado_por, :idLogin)";
    ...
    if ($result) {
        ...
        f_log("INC", "Incluiu Membro de CIPA: $stringDados", "rh_cipeiros", $idModulo, $idMembro);
        $retorno = "Inclusão bem-sucedida!";
        $status = true;
        //
        $idPessoa = $dados['idPessoa'];
        $tipo = 34; // Evento de CIPA
        $descricao = "Inserido como Membro da CIPA";
        f_ldt( $tipo, $idPessoa, $descricao);
        //          
    } else {
        // Ocorreu um erro durante a inserção
        $retorno = "Erro ao inserir Membro de CIPA.";
        $status = false;
