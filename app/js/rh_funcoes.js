...
function f_incluir_commit() {
    ...
    if ($("#_descricao").val() === "") {
        alert("Por favor, informe a descrição da função.");
        $("#_descricao").focus();
        return false;
    }
    //
    ...
    $.ajax({
        url: "includes/rh_funcao_aj2.php",
        type: "POST",
        data: formData,
...
function f_excluir(id) {
    if (confirm("Tem certeza que deseja excluir o registro de ID " + id + "?")) {
        $("#divAlertaFuncao").show();
        $.post("includes/rh_funcao_aj3.php", { id: id }, function (response) {
            const dados = JSON.parse(response);
            //alert(response);
            $("#divAlertaFuncao").html(dados.msg);
            //
            setTimeout(function () {
                $("#divAlertaFuncao").html("");
                selecionou();
            }, 3000);
...
function f_editar(id) {
    //
    $.post("includes/rh_funcao_aj1.php", { id: id }, function (retorno) {
        const dados = JSON.parse(retorno);
        altModal.show();
        $("#formEditar #e_idFuncao").val(id);
...
function f_editar_commit() {
    ...
    // 🔥 3. ENVIANDO O FORMULÁRIO
    $.ajax({
        url: "includes/rh_funcao_aj4.php",
        type: "POST",
        data: formData,
        processData: false,  // Evita que o jQuery tente converter o FormData em string
        contentType: false,  // Permite que arquivos sejam enviados corretamente
...
function f_visualizar(id) {
    ...
    $.post("includes/rh_funcao_aj1.php", { id: id, origem: 'visualizar' }, function (retorno) {
        const dados = JSON.parse(retorno);
...
function f_reset() {
    //
    let id = $("#formEditar #e_idFuncao").val();
    //
    $.post("includes/rh_funcao_aj1.php", { id: id }, function (retorno) {
        const dados = JSON.parse(retorno);
        altModal.show();
