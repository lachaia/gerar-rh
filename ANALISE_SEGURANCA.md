...
10. ⬜ Revisar sistematicamente os arquivos que usam `extract($_POST)`/`extract($dados)` e trocar por atribuição explícita + bind de parâmetro em toda query. *(o SQLi de CIPA/Brigada/Termos e dos IDORs de ponto/colaborador/gestor já foi fechado; o restante — `rh_docs_edt_aj.php`/`rh_docs_eml_aj.php` — segue pendente, ver seção ALTO.)*
11. ⬜ Adicionar token CSRF (ou pelo menos `SameSite=Strict/Lax` nos cookies de sessão) como camada adicional.
