<?php
//
//- vaga_fluxo_inc_aj.php | Inclui novo registro em rs_vagas_fluxo
//- (C)haia, 26/08/2026
//
...
$sql = "INSERT INTO rs_vagas_fluxo (status, descricao, ativo, cor_frente, cor_fundo)
...
if ($stmt->execute()) {
    echo json_encode(["status" => true, "msg" => "Etapa incluída com sucesso.", "id" => $conn->lastInsertId()]);
} else {
    echo json_encode(["status" => false, "msg" => "Falha ao incluir etapa."]);
}
exit;
