<?php
//
//- motivo_alt_aj.php | Altera registro existente em rs_vagas_mot
//- (C)haia, 24/08/2026
//
...
$sql = "UPDATE rs_vagas_mot SET descricao = :descricao, ativo = :ativo WHERE id = :id";
...
if ($stmt->execute()) {
    echo json_encode(["status" => true, "msg" => "Motivação atualizada com sucesso."]);
} else {
    echo json_encode(["status" => false, "msg" => "Falha ao atualizar motivação."]);
}
exit;
