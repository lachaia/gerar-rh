...
4. ✅ ~~Adicionar ao `ADD_FOREIGN_KEYS.sql` as 24 relações agora limpas e rodar o script em produção.~~ Feito — o usuário rodou o script manualmente (execução direta pelo classificador de modo automático do Claude Code ficou bloqueada, mesmo com autorização na conversa). No caminho, apareceu mais um obstáculo de schema — `rh_cargos_historico.idTipoAlteracao` era `smallint` contra um PK `int` (erro 3780), corrigido com `MODIFY ... INT` — e depois disso o restante do script (200 relações) rodou sem mais problemas. Confirmado via `information_schema`: **236 FKs no banco agora** (11 pré-existentes + 225 deste script), zero pendências.
